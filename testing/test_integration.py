...
All 6 connect_* functions are patched so no network is needed,
but real threading runs.
"""
...
def mock_connections(monkeypatch):
    """Patch all 6 connect_* functions on the arista module.
...
def test_run_benchmark_collects_all_six(sample_params, mock_connections):
    """All 6 methods called, all results collected, sorted by time."""
    returns = [
        _result("SSH", 0.50), _result("eAPI", 0.10), _result("RESTCONF", 0.30),
        _result("NETCONF", 0.60), _result("gNMI", 0.20), _result("Telnet", 0.40),
    ]
    ...
    assert {r.method for r in results} == {"SSH", "eAPI", "RESTCONF", "NETCONF", "gNMI", "Telnet"}
...
def test_run_benchmark_handles_thread_exception(sample_params, mock_connections):
    ...
    good = [
        ...
        _result("NETCONF", 0.6), _result("Telnet", 0.4),
    ]
    ...
    mock_connections["connect_telnet"].return_value = good[4]
